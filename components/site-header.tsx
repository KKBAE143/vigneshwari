...
export function SiteHeader() {
  ...
  return <header className="sticky top-0 z-50 bg-background/95 backdrop-blur-md">
    ...
    <div className="site-container flex h-20 items-center justify-between border-b border-border lg:h-24">
      ...
      <div className="hidden items-center gap-5 lg:flex"><Button variant="ghost" size="icon" render={<Link href="/search" aria-label="Search" />}><Search /></Button><span className="text-xs uppercase tracking-[.18em]">India / INR</span></div>
      <Link href="/" className="font-serif text-3xl tracking-[.08em] text-primary md:text-4xl">VIGNESHWARI</Link>
      <div className="flex items-center gap-1"><Button variant="ghost" size="icon" render={<Link href="/wishlist" aria-label={`Wishlist with ${wishlist.length} items`} />}><Heart /></Button><Button variant="ghost" size="icon" render={<Link href="/cart" aria-label={`Bag with ${cart.length} items`} />}><ShoppingBag /></Button></div>
    </div>
